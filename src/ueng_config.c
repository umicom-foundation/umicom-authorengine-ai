...
static void set_env_if(const char* name, const char* value, int overwrite) {
  ...
  if (existing && *existing && !overwrite) return;
  /* Set it */
...
static void apply_kv(UengConfig* c, const char* key, const char* value) {
  ...
  if (strcmp(key, "llm.provider") == 0) {
    ...
    if (v > 0 && v < 65536) c->serve_port = (int)v;
...
int ueng_config_load_file(UengConfig* c, const char* path) {
  if (!c || !path || !*path) return 1;
  FILE* f = fopen(path, "r");
  if (!f) return 2; /* not found or not readable; keep it non-fatal to caller */
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (!colon) continue; /* ignore malformed lines */
    *colon = '\0';
    char* key = line;
    char* val = colon + 1;
    trim_in_place(key);
...
void ueng_config_apply_env(UengConfig* c) {
  ...
  if ((s = getenv("UENG_LLM_PROVIDER")) && *s) copy_str(c->llm_provider, sizeof(c->llm_provider), s);
  if ((s = getenv("UENG_LLM_MODEL"))    && *s) copy_str(c->llm_model,    sizeof(c->llm_model),    s);

  if ((s = getenv("OPENAI_API_KEY"))        && *s) copy_str(c->openai_api_key,  sizeof(c->openai_api_key),  s);
  if ((s = getenv("UENG_OPENAI_BASE_URL"))  && *s) copy_str(c->openai_base_url, sizeof(c->openai_base_url), s);

  if ((s = getenv("UENG_OLLAMA_HOST"))      && *s) copy_str(c->ollama_host, sizeof(c->ollama_host), s);
  if ((s = getenv("UENG_LLAMA_MODEL_PATH")) && *s) copy_str(c->llama_model_path, sizeof(c->llama_model_path), s);

  if ((s = getenv("UENG_SERVE_HOST")) && *s) copy_str(c->serve_host, sizeof(c->serve_host), s);
  if ((s = getenv("UENG_SERVE_PORT")) && *s) {
    long v = strtol(s, NULL, 10); if (v > 0 && v < 65536) c->serve_port = (int)v;
  }

  if ((s = getenv("UENG_WORKSPACE_DIR")) && *s) copy_str(c->workspace_dir, sizeof(c->workspace_dir), s);
  if ((s = getenv("UENG_SITE_ROOT"))     && *s) copy_str(c->site_root,     sizeof(c->site_root),     s);
}
