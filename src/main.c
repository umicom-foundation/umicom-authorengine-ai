...
static int parse_bool_line(const char* line, const char* key, int* out_bool){
  ...
  if (strcmp(buf,"true")==0 || strcmp(buf,"1")==0 || strcmp(buf,"yes")==0 || strcmp(buf,"on")==0){
    *out_bool = 1; return 1;
  }
  if (strcmp(buf,"false")==0 || strcmp(buf,"0")==0 || strcmp(buf,"no")==0 || strcmp(buf,"off")==0){
    *out_bool = 0; return 1;
  }
  return 0;
}
...
static int light_export_html(const char* title, const char* author,
                             const char* html_dir, const char* md_path,
                             char* out_html, size_t out_html_sz)
{
  if (mkpath(html_dir) != 0){
    fprintf(stderr,"[export] ERROR: cannot create %s\n", html_dir);
    return 1;
  }
  char rel_css[32]; rel_css[0]='\0';
  ...
  if (!in){ fprintf(stderr,"[export] ERROR: missing %s\n", md_path); return 1; }
  FILE* out = ueng_fopen(out_html, "wb");
  if (!out){ fclose(in); fprintf(stderr,"[export] ERROR: cannot write %s\n", out_html); return 1; }
  ...
  fputs("<body style=\"margin:2rem auto;max-width:860px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Arial,sans-serif;line-height:1.6\">", out);
...
static int cmd_init(void){
  (void)write_text_file_if_absent("book.yaml",
    "title: My New Book\n"
    "author: Anonymous\n"
    "ingest_on_build: false\n");

  BookCfg cfg; read_book_cfg(&cfg);
  char slug[256]; slugify(cfg.title, slug, sizeof(slug));
...
static int cmd_build(void){
  ...
  char root[640]; snprintf(root,sizeof(root),"outputs%c%s%c%s", PATH_SEP, slug, PATH_SEP, day);
  mkpath(root);

  /* Common subfolders we expect to populate. */
  const char* sub[] = {"pdf","docx","epub","html","md","cover","video-scripts","site",NULL};
  for (int i=0; sub[i]; ++i){
    char p[640]; snprintf(p,sizeof(p),"%s%c%s",root,PATH_SEP,sub[i]); mkpath(p);
  }
  ...
  char rel_css_tmp[16]; rel_css_tmp[0]='\0';
  (void)copy_theme_into_html_dir(html_dir, rel_css_tmp, sizeof(rel_css_tmp));

  /* Make a simple site landing page with links. */
  char stamp[64]; time_t now=time(NULL); struct tm* tm=gmtime(&now);
  strftime(stamp,sizeof(stamp),"%Y-%m-%d %H:%M UTC", tm);
  char site_dir[640]; snprintf(site_dir,sizeof(site_dir),"%s%csite", root, PATH_SEP);
  if (write_site_index(site_dir, cfg.title, cfg.author, slug, stamp, /*has_cover=*/1, has_draft) != 0){
     ...
     users can open something immediately. */
  {
    ...
    if (!file_exists(out_html) && file_exists("workspace/book-draft.md")){
      (void)light_export_html(cfg.title, cfg.author, html_dir, "workspace/book-draft.md", out_html, sizeof(out_html));
    }
  }
...
static int cmd_export(void){
  ...
  char out_root[640]; snprintf(out_root,sizeof(out_root),"outputs%c%s%c%s", PATH_SEP, slug, PATH_SEP, day);
  char html_dir[640]; snprintf(html_dir,sizeof(html_dir), "%s%chtml", out_root, PATH_SEP);
  mkpath(html_dir);
...
static int cmd_serve(int argc, char** argv){
  ...
  if (i < argc){
    const char* a = argv[i];
    int is_port = 1;
    for (const char* p=a; *p; ++p) if (!isdigit((unsigned char)*p)) { is_port = 0; break; }
    if (strchr(a,'.') || strchr(a,':')) is_port = 0;
    if (is_port){
      port = atoi(a);
      ++i;
    }else{
  ...
  if (!site_root || !*site_root){
    ...
    char slug[256]; slugify(cfg.title, slug, sizeof(slug));
    char day[32];   build_date_utc(day, sizeof(day));
    snprintf(calc_root,sizeof(calc_root),"outputs%c%s%c%s%csite", PATH_SEP, slug, PATH_SEP, day, PATH_SEP);
    site_root = calc_root;
  }
...
static int cmd_doctor(void){
  ...
  puts(has_pandoc ? "[ok] pandoc found on PATH" : "[info] pandoc not found (HTML export will use light fallback)");
  ...
  puts(has_headless ? "[ok] Edge/Chrome present for headless PDF" : "[info] No headless Chrome found (PDF via pandoc only)");
#endif
...
int main(int argc, char** argv){
  ...
  else if (strcmp(cmd,"init")==0)        { return cmd_init(); }
  ...
  else if (strcmp(cmd,"serve")==0)       { return cmd_serve(argc-2, argv+2); }
  else if (strcmp(cmd,"open")==0)        { return cmd_open(); }
  else if (strcmp(cmd,"render")==0)      { return cmd_render(); }
