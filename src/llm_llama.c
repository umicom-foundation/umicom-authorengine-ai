...
ueng_llm_ctx* ueng_llm_open(const char* model_path, int ctx_tokens,
    ...
    if (!model_path || !*model_path) {
        if (err && errsz) snprintf(err, errsz, "model path is empty");
        return NULL;
    }
    if (ctx_tokens <= 0) ctx_tokens = 4096;
...
int ueng_llm_prompt(ueng_llm_ctx* handle, const char* prompt, char* out, size_t outsz) {
    ...
    if (llama_decode(R->ctx, batch) != 0) { llama_batch_free(batch); return -3; }
    ...
    for (int t = 0; t < 64 && used + 8 < outsz; ++t) {
        ...
        if (llama_decode(R->ctx, batch) != 0) break;
    }
...
ueng_llm_ctx* ueng_llm_open(const char* model_path, int ctx_tokens,
                            char* err, size_t errsz) {
    (void)model_path; (void)ctx_tokens;
    if (err && errsz) {
        snprintf(err, errsz,
