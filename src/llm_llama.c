...
static int env_backend(void)
{
  ...
  if (ueng_stricmp(e, "openai") == 0)
    return UENG_LLM_BACKEND_OPENAI;
  if (ueng_stricmp(e, "ollama") == 0)
    return UENG_LLM_BACKEND_OLLAMA;
  if (ueng_stricmp(e, "llama") == 0)
    return UENG_LLM_BACKEND_LLAMA;
  return UENG_LLM_BACKEND_LLAMA;
}
...
ueng_llm_ctx *ueng_llm_open(const char *model_or_id, int ctx_tokens, char *err, size_t errsz)
{
  ...
  if (be == UENG_LLM_BACKEND_OPENAI)
  {
    ...
    if (ctx)
      ctx->backend = UENG_LLM_BACKEND_OPENAI;
    return ctx;
  }
#else
  if (be == UENG_LLM_BACKEND_OPENAI)
  ...
  if (be == UENG_LLM_BACKEND_OLLAMA)
  {
    ...
    if (ctx)
      ctx->backend = UENG_LLM_BACKEND_OLLAMA;
    return ctx;
  }
#else
  if (be == UENG_LLM_BACKEND_OLLAMA)
