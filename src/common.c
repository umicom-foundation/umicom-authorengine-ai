...
void slugify(const char* in, char* out, size_t out_sz){
  ...
  for (const unsigned char* p=(const unsigned char*)in; *p && j+1<out_sz; ++p){
    ...
    if (isalnum(c)){
      ...
      if (!need_dash && j>0){ out[j++]='-'; need_dash=1; }
    } else {
      if (!need_dash && j>0){ out[j++]='-'; need_dash=1; }
    }
  }
...
int mkpath(const char* p){
  ...
  for (char* q=tmp+1; *q; ++q){
    if (*q=='\\'){
      char save=*q; *q='\0';
      if (!dir_exists(tmp)) if (make_dir(tmp)!=0 && !dir_exists(tmp)) return -1;
      *q=save;
    }
  }
  if (!dir_exists(tmp)) if (make_dir(tmp)!=0 && !dir_exists(tmp)) return -1;
  ...
  for (char* q=tmp+1; *q; ++q){
    if (*q=='/'){
      *q='\0';
      if (!dir_exists(tmp)) if (mkdir(tmp,0755)!=0 && !dir_exists(tmp)) return -1;
      *q='/';
    }
  }
  if (!dir_exists(tmp)) if (mkdir(tmp,0755)!=0 && !dir_exists(tmp)) return -1;
...
int read_text_file(const char* path, char* out, size_t outsz){
  FILE* f=ueng_fopen(path,"rb"); if (!f) return -1;
...
int write_gitkeep(const char* dir){
  ...
  char p[PATH_MAX]; snprintf(p,sizeof(p),"%s%c.gitkeep",dir,PATH_SEP);
...
int clean_dir(const char* dir){
  ...
  while ((e=readdir(d))){
    ...
    if (S_ISDIR(st.st_mode)){ clean_dir(p); rmdir(p); }
    else unlink(p);
  }
  closedir(d);
  return 0;
...
int open_in_browser(const char* path_or_url){
  if (!path_or_url || !*path_or_url) return -1;
  ...
  if (path_abs(path_or_url, abs, sizeof(abs)) != 0) return -1;
