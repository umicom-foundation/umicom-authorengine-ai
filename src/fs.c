...
int copy_theme_into_html_dir(const char* html_dir, char* rel_css, size_t rel_css_sz){
  if (mkpath(html_dir) != 0) return -1;
  char css[PATH_MAX];
  snprintf(css, sizeof(css), "%s%cstyle.css", html_dir, PATH_SEP);
  const char* minimal_css =
    "body{color:#111;background:#fff}"
    "h1,h2,h3{line-height:1.25}"
    "pre{white-space:pre-wrap}";
  if (write_text_file_if_absent(css, minimal_css) != 0) return -1;
  if (rel_css && rel_css_sz){ strncpy(rel_css, "style.css", rel_css_sz-1); rel_css[rel_css_sz-1]='\0'; }
  return 0;
}
...
int pack_book_draft(const char* title, const char* outputs_root, int* out_has_draft){
  ...
  FILE* out = ueng_fopen(draft, "wb"); if (!out) return -1;
...
int generate_cover_svg(const char* title, const char* author, const char* slug){
  ...
  snprintf(buf, sizeof(buf),
    ...
    "<text x=\"50\" y=\"200\" font-size=\"64\" font-family=\"Segoe UI,Arial,sans-serif\">%s</text>\n"
    "<text x=\"50\" y=\"300\" font-size=\"32\" font-family=\"Segoe UI,Arial,sans-serif\">by %s</text>\n"
    "</svg>\n", title, author);
...
int generate_frontcover_md(const char* title, const char* author, const char* slug){
  (void)slug;
  (void)mkpath("workspace");
  char path[PATH_MAX]; snprintf(path, sizeof(path), "workspace%cfrontcover.md", PATH_SEP);
  char buf[2048];
  snprintf(buf, sizeof(buf),
    "# %s\n\n"
    "_by %s_\n\n"
...
int write_site_index(const char* site_dir, const char* title, const char* author,
                     const char* slug, const char* stamp,
                     int has_cover, int has_draft){
  if (mkpath(site_dir) != 0) return -1;
